...
| S-63b | Antithesis | **Trigger** You or an ally within 30 feet takes damage from a creature you can see | `test-riders` pins the reach and where each half lands | ✅ | **Fixed; the module had no shape for it, and now it has one.** *“You … take damage”* always worked; *“or an ally within 30 feet”* could not, because `damage-received` is the **defender's own** event — when the ally is hit the engine reads the *ally's* items and the Quincy's Technique is not among them. The new `ally-damaged` event is dispatched to each ally within the rider's own `range`, and it is the only event in the module with **three** participants: the watcher, the striker, and the creature that was hurt. Live at 25 feet: an NPC hit the Quincy's ally, the Quincy was offered the reaction, the **2d6 went to the striker** (D1 353 → 336) and **`Effect: Antithesis — Reversed`** landed on the **ally**, reading `all-damage 13` at 13th level — not on the Quincy. `trigger: "ally"` is that third address; `trigger: true` still means the striker, so the spirit damage reads identically on both halves. *“From a creature you can see”* is still unchecked, as every perception clause here is |
